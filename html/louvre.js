...
const louvre = async ({ img, outputCanvas, config, callback }) => {
  ...
  if (config.shade) {
    ...
    for (let i = 0; i < shadePixel.data.length; i += 4) {
      ...
      ); //((255 - pencilTexturePixel.data[i]) + Math.random() * 40 - 20);
...
function gpuConv(pixels, weights, w, h, side, halfSide) {
  // let r = 0;
  const idx = ((this.thread.y) * w + this.thread.x) * 4;
  this.color(pixels[idx] / 255, pixels[idx + 1] / 255, pixels[idx + 2] / 255, pixels[idx + 3] / 255);

  // for (let cy = 0; cy < side; cy++) {
  //   for (let cx = 0; cx < side; cx++) {
  //     const scy = Math.min(h - 1, Math.max(0, this.thread.y + cy - halfSide));
  //     const scx = Math.min(w - 1, Math.max(0, this.thread.x + cx - halfSide));
  //
  //     const srcOff = (scy * w + scx) * 4;
  //     const wt = weights[cy * side + cx];
  //
  //     r += pixels[srcOff] * wt;
  //     if (this.thread.y == 688 && this.thread.x == 488) {
  //       // console.log('expected', r, ++i)
  //       // return r;
  //     }
  //
  //   }
...
const convoluteY = (pixels, weights, ctx) => {
  ...
  const convoluteYGPU = gpu.createKernel(gpuConv).setOutput([
    pixels.width,
    pixels.height,
  ])
    .setPrecision('single')
  ...
  console.log(JSON.stringify(
    convoluteYGPU(
      ...
      side,
      halfSide,
    )?.map(x => x.filter(e => e)).filter(x => x.length))
  )

  convoluteYY(pixels, weights, ctx)

  return new ImageData(convoluteYGPU.getPixels(), pixels.width, pixels.height);
};

let i = 9

// const convoluteYY = (pixels, weights, ctx) => {
const convoluteYY = (pixels, weights, ctx) => {
  const side = Math.round(Math.sqrt(weights.length));
  const halfSide = Math.floor(side / 2);
  ...
  console.log("convolute", side, h, w);
